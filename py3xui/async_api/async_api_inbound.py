...
class AsyncInboundApi(AsyncBaseApi):
    ...
    Public Methods:
        ...
        enable: Enables an inbound.
        disable: Disables an inbound.
    ...
    async def get_list(self) -> list[Inbound]:
        ...
        Returns:
            list[Inbound]: A list of Inbound objects representing the inbounds.
    ...
    async def enable(self, inbound_id: int) -> None:
        ...
        await api.inbound.enable(inbound.id)
        
        """
        endpoint = f"panel/api/inbounds/enable/{inbound_id}"
        headers = {"Accept": "application/json"}

        url = self._url(endpoint)
        data: dict[str, Any] = {}

        self.logger.info("Enabling inbound with ID: %s", inbound_id)
        await self._post(url, headers, data)
    ...
    async def disable(self, inbound_id: int) -> None:
        ...
        await api.inbound.disable(inbound.id)
        
        """
        endpoint = f"panel/api/inbounds/disable/{inbound_id}"
        headers = {"Accept": "application/json"}

        url = self._url(endpoint)
        data: dict[str, Any] = {}

        self.logger.info("Disabling inbound with ID: %s", inbound_id)
        await self._post(url, headers, data)
        self.logger.info("Inbound disabled successfully.")